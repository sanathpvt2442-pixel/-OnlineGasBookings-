...
export default function Admin() {
	...
	const handleAdd = (e) => {
		e.preventDefault();
		setOpenAdd(false);
		axios.post("http://localhost:4000/api/insertRow", { table: Tables[buttonSelected].Tables_in_gasbooking, row: dialogData })
			.then((res) => {
				handleTable(Tables[buttonSelected].Tables_in_gasbooking);
			})
			.catch((err) => {
	...
	return (
		<>
			{
				(id !== 'chhavi' || name !== "chhavi") ? console.log('page Not Found') :
